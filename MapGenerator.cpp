...
void Room::generate(bool exit, bool chest, bool enemy,std::vector<std::vector<char>> *map,bool boss)
    {
        ...
        if (enemy) {
            ...
            std::shared_ptr<Enemy> enemy = std::make_shared<Enemy>(new Enemy(enemy_coords, map, this->node,boss?true:false));
            ...
            if (rand() % 4 == 3) {
                ...
                if (enemy_coords != enemy2_coords) {
                    std::shared_ptr<Enemy> enemy2 = std::make_shared<Enemy>(new Enemy(enemy_coords, map, this->node, false));
